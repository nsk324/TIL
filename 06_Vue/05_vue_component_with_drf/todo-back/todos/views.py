...
def todo_create(request):
    ...
    if serializer.is_valid():
        serializer.save()
        # serializer.data => Json ({'id':1,'user':1,'title':첫번째할일,'completed':flase})
...
def todo_update_delete(request,id):
    ...
    if request.method == 'PUT':
        #기존 todo에서 request.POST(수정내용)으로 변경
    ...
    #유효하지 않으면 에러 메시지와 함께 400에러
        ...
        todo.delete()
        # 204 -> 해당하는 컨텐츠가 없는 경우 (todo 를 삭제 했기 때문에 해당하는 todo가 존재하지 않음을 알려줌)
